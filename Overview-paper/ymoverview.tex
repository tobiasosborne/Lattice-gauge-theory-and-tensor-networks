...
\end{equation}
where $l$ indexes the irreps, $V_l$ denotes the vector space furnishing the irreducible representation $t^l$, and $V^*_l$ its dual.
...
\end{equation}
Thanks to Eq.~(\ref{eq:peterweyl}) we know that $L^2(\su2)$ is the direct sum structure of bipartite hilbert spaces thus it may be regarded, \emph{for all practical purposes}, as the state space of a bipartite quantum system. This is one point where $\su2$ and $\uone$ differ: crucially we do \emph{not} have this bipartite structure in the $\uone$ case because all of its irreps are one dimensional. In the $\su2$ case we visualise the bipartite-like structure graphically by adding virtual vertices to the ends of a link, intended to represent these two different subsystems: we now picture an arbitrary state $|\psi\rangle$ of a single link as  
\begin{center}
\includegraphics{edgespace.pdf}
\end{center}
According to this observation we extend our original lattice representation by associating the degrees of freedom at the ends of a link with auxiliary vertices associated with the lattice locations:
\begin{center}
...
\begin{equation}
	\mathcal{H}_v \equiv \bigoplus_{l_1,l_2,l_3,l_4\in\frac12\mathbb{Z}} V_{l_1}^*\otimes V_{l_2}^*\otimes V_{l_3}\otimes V_{l_4},
\end{equation}
...
\begin{equation}
	\mathcal{H} \equiv \bigotimes_{e\in E} L^2(\su2) \cong \bigotimes_{v\in V} \mathcal{H}_v.
\end{equation}
...
Consider the Lie algebra $\lasu2$ of \su2:
\begin{equation}
...
\begin{equation}
	[\widehat{\ell}_L, \widehat{u}_{jk}]= - [\sigma^\alpha \widehat{u}]_{jk}.
\end{equation} 
...
In many ways this is a remarkably simple model: it comprises only two really different terms, a ``kinetic energy'' term $\text{KE} \equiv -\sum_{e\in E} \triangle_e$ and a ``potential energy'' term $\text{PE} \equiv \sum_{\square} 2 -\text{Re}(\tr(\widehat{u}_{\cwplaqsub}))$, both of which can be exactly diagonalised. The $\text{KE}$ term is diagonal in the $|jk\rangle_l$, $l\in\frac12\mathbb{Z}^+$, $j,k = -l,\ldots, l$, ``momentum'' basis and the $\text{PE}$ term is diagonal in conjugate ``position'' basis $|U\rangle$, $U\in \su2$.
...
Although we are working in the temporal gauge there is still a residual gauge freedom \cite{creutz:1985a}. This freedom is expressed in terms of the \emph{gauge group} $\mathcal{G}$ which is the cartesian product of $\su2$ over all the lattice points, $\mathcal{G}\cong \prod_{v\in V} \su2$, and which is represented on $\mathcal{H}$ by 
\begin{equation}
...
The most important gauge-invariant state is built from the trivial representation of $\su2$ and is given by
\begin{equation}
	|\omega_0\rangle = \int |U\rangle\, dU.
\end{equation}
The wavefunction for this state is simply the constant function corresponding to the basis vector $|00\rangle_0 \cong t^0_{00}(g)$. 
Note that left and right invariance of the haar measure implies that
\begin{equation}
	L_U|\omega_0\rangle = R_U|\omega_0\rangle = |\omega_0\rangle, \quad \forall U\in \su2.
\end{equation}
...
\end{center}
The hilbert space for this system is $L^2(\su2)$ and the gauge-invariant sector $\mathcal{H}_{\mathcal{G}}$ is found as follows. The local gauge group $\mathcal{G} \cong \su2$ acts as
\begin{equation}
	|\psi\rangle \mapsto L_xR_x |\psi\rangle, \quad \forall x\in \su2.
\end{equation}
Thus a state $|\psi\rangle$ is gauge invariant if and only if
\begin{equation}\label{eq:classstate}
	\int \psi(U) |U\rangle\, dU = \int  \psi(x^{-1}Ux) |U\rangle\, dU, \quad\forall x\in \su2.
\end{equation}
Because $\mathcal{G}$ acts \emph{irreducibly} $\psi$ must be a \emph{class function}, i.e., $\psi(x^{-1}Ux) = \psi(U)$. 

A generalisation of the single-loop graph is the \emph{petal} graph 
\begin{center}
	\includegraphics{petal.pdf}
\end{center}
comprising a single vertex and $n$ edges. The configuration space for this system is $\mathcal{H} = \bigotimes_{j=1}^n L^2(\su2)$. The local gauge group $\mathcal{G}$ is again isomorphic to a single copy of $\su2$ and now acts as a tensor product:
\begin{equation}
...
Since $\mathcal{G}$ now acts \emph{reducibly} the gauge-invariant sector is no longer spanned by states whose wavefunctions are class functions; instead a gauge invariant state for the petal graph is equivalent to a superposition of \emph{intertwiners} for $\su2$ \cite{baez:1996a}. 
...
Since this entire process was isometric we  see that the gauge-invariant subspace of $\mathcal{H}$ is equivalent to space of gauge invariant states of a petal graph, which, in turn is given by the space of intertwiners. An illustration of this procedure is shown in Fig.~\ref{fig:gis}. 
...
As a warm up we first consider the problem of interpolation for a tiny ``lattice'' of two points and two edges. Here the task is to interpolate between the parallel transporters, $U_0$ and $U_n$, on the two edges:
\begin{center}
...
\begin{equation}
	A_j, \quad j = 0, 1, 2, 3,
\end{equation}
the elements of $\su2$ found using the interpolation procedure of the previous subsection applied to the tuple $(U(\gamma_0), U(\gamma_1), U(\gamma_2), U(\gamma_3))$. Finally, we introduce the controlled unitary 
\begin{equation}
...
\begin{equation}
	|\Psi'\rangle \equiv \textsl{C}\mathcal{V}_{\psi} |\Psi\rangle,
\end{equation}
...
We now begin the discussion of how to take the continuum limit. Conditioned on the validity of the previously mentioned conjectures, we've produced a sequence of states $|\Psi_m\rangle$ which tend, in the limit, to the zero-coupling state. The important observation here is that since each term in our sequence is a MERA the correlation length of $|\Psi_m\rangle$ is given by $\xi_m = a_m\lambda^m$, for some $\lambda>0$.  Given that the correlation length $\xi$ of pure Yang-Mills theory is only determined up to a constant which is ultimately fixed by experiment forces us to set the lattice spacing $a_m$ of the state $|\Psi_m\rangle$ to $a_m = a_0\lambda^{-m}$, where $a_0$ is a constant. Thus we have a sequence of states $|\Psi_m\rangle$ for lattices of ever finer discretisation. Further, we can compute all $n$-point functions for this sequence. This is exactly the input required for the continuum limit procedure described in the next section.


Here we discuss the approach to the continuum of the Kogut-Susskind model in the scaling limit $g_H\rightarrow 0$. The objective here is to first derive an effective quantum field model for the low-energy physics of the scaling limit and then to check whether this model exhibits the properties expected of a nonabelian gauge theory. 

The key to understanding continuum limits of lattice models in the hamiltonian formalism is to first agree on a limited set  observables, being discretisations of corresponding field operators, and to ignore all other observables. We then calculate the expectation values of these observables and if they converge as the lattice spacing is reduced to zero then we declare these expectation values to be those of the correspond quantum field operators. 
...
\end{document}
\section{The u1 case}
In the abelian case $G \cong \uone$ with
\begin{equation}
   \uone = \left\{ z \middle| z = e^{i\theta}, -\pi \le \theta < \pi \right\},
\end{equation}
the space $L^2(\uone)$ is that of the functions $\phi(\theta)$
such that $\langle \phi | \phi \rangle = \frac{1}{2\pi}\int_{-\pi}^{\pi} d\theta \, \overline{\phi(\theta)} \phi(\theta)$ is
finite. The irreducible representations of $\uone$ are given by
$t^l \cong e^{il\theta}$ with $l \in \mathbb{Z}$. We exploit the notation
\begin{equation}
  |l\rangle \cong e^{il\theta}
\end{equation}
and decompose functions $\phi(\theta)$ via
\begin{equation}
  |\phi\rangle = \sum_l \widehat{\phi}^l |l\rangle,
\end{equation}
where $\widehat{\phi}^l$ are the fourier coefficients of $\phi(\theta)$
\begin{equation}
  \widehat{\phi}^l = \langle l|\phi\rangle = \frac{1}{2\pi} \int_{-\pi}^{\pi} d\theta \, e^{-il\theta} \phi(\theta).
\end{equation}
We define the \emph{position observable} $\widehat{u}$ via
\begin{equation}
  \widehat{u}|\theta\rangle = e^{i\theta} |\theta\rangle.
\end{equation} 

The Lie algebra $\lauone$ consists of the antihermitian $1 \times 1$ matrices.
A basis is given by the imaginary unit $i$ and elements of $\lauone$ correspond to infinitesimal rotations: setting $U = e^{i\epsilon}$ we obtain the 
infinitesimal left and right rotations
\begin{equation}
  \widehat{\ell}_L \equiv \frac{d}{d\epsilon}L_{e^{i\epsilon}} |_{\epsilon = 0}, \quad \text{and}\quad \widehat{\ell}_R \equiv \frac{d}{d\epsilon}R_{e^{i\epsilon}} |_{\epsilon = 0}.
\end{equation}

The matrix elements of $\widehat{\ell}_L$ and $\widehat{\ell}_R$
in terms of the fourier basis are
\begin{equation}
  \langle l|\widehat{\ell}_L |m\rangle = \int_{-\pi}^{+\pi} \frac{d\theta}{2\pi}
    e^{-il\theta} \frac{d}{d\epsilon} e^{im(\theta - \epsilon)} \bigg|_{\epsilon = 0}
    = -il\delta_{lm},
\end{equation}
and, similarly, $\langle l|\widehat{\ell}_R |m\rangle = il\delta_{lm}$. 

\section{Block spin transformations}

natural classes of gauge invariant tensor networks built using the primitives of edge and vertex addition. The main idea in this section is that there are two important states which admit an efficient exact description in terms of tree tensor networks, namely, the strong coupling fixed point and the zero-coupling limit. The strong coupling limit is simply $|\Omega_0\rangle$, which is a trivial tensor network. The zero-coupling limit is a tree tensor network and is an example of a \emph{flat} state of the gauge field. Thus both of these states are \emph{fixed points} of the renormalisation group. In order to see this we will develop the technology of block spin averaging and its inverse, \emph{interpolation}. Using block spin transformations we'll show how the strong and zero-coupling limits are RG fixed points.

We develop our discussion of the renormalisation group by explaining, in operational terms, some of the additional complexities presented by the gauge invariant sector. The constraint of gauge invariance means that we are only able to measure observables which are themselves gauge invariant. Thus, the space of all observables for our system is no longer the space of \emph{arbitrary} operators on $\mathcal{H}$ which forms, conveniently an algebra, but rather, some subalgebra of \emph{gauge-invariant observables}. This space is a little more difficult to characterise but there are at least three equivalent ways to do this, namely via: (a) Wilson loops; (b) spin networks; or (c) by adding vertices. The first algebra of observables, $\mathcal{A}_W$, is that generated by all observables of the form $\tr(\widehat{u}_\gamma)$, as $\gamma$ runs over all possible closed loops on the lattice (with repeated edges allowed) \cite{wilson:1974b}. It is clear that the algebra of all such observables are locally gauge invariant by construction. The second way to define locally gauge invariant observables is to consider the algebra of position-space observables generated by the following recipe: choose an irrep. $V_e$, one per link $e\in E$, labelled by a half-integer $\ell(e)$; next choose an \emph{intertwiner}, which is a linear map on $\bigotimes_{e\in E} V_e$ which commutes with the action of the local gauge group $\mathcal{G}$; then the function $\mathcal{S}(g_{e_1}, g_{e_2}, \ldots) \equiv \tr(\pi(g_{e_1})\otimes \cdots \times I)$ is locally gauge invariant. Such functions are known as \emph{spin networks} \cite{baez:1996a}. It turns out \cite{levy:2004a} that these two algebras are, in fact, equivalent.


Explanation of the observables.

Now that we've characterised the operationally meaningful observables we can discuss what is meant by block-spin renormalisation. This operation is, at its heart, simply a way to \emph{discard} information about features on the lengthscale of a block. This is achieved using a \emph{completely positive} map (CP) map $\mathcal{E}$, which is the most general linear operation allowed in quantum mechanics. The usual example of the RG in the lattice context is afforded by Kadanoff block decimation, i.e., the simple \emph{partial trace} operation. Unfortunately, however, this RG is not available to us as it breaks local gauge invariance. Thus there is a tension between the requirement of gauge invariance and the discarding of information in gauge theories. We resolve this tension by focusing on its action of the gauge-invariant observable algebra instead, where gauge invariance is manifest.

It is difficult to describe our RG in the Schr\"odinger picture as the requirement of complete positivity can be difficult to enforce, however, in the Heisenberg picture there is a simple operationally clear definion: $\mathcal{E}^\times$ is the \emph{restriction} of the algebra $\mathcal{A}_W$ to the algebra of wilson loops defined with respect to paths on a \emph{sublattice} (or, in the general case, a \emph{subgraph}). In the general case that concerns us here, namely the regular lattice $a\mathbb{Z}^d$, the sublattice in question is simply $2a\mathbb{Z}^d$. This linear map is manifestly CP and trace preserving. It is also 
