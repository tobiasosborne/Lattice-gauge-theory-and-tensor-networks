...
\end{equation}
Thanks to Eq.~(\ref{eq:peterweyl}) we know that $L^2(\su2)$ has the structure of a direct sum of bipartite hilbert spaces thus it may be regarded, \emph{for all practical purposes}, as the state space of a bipartite quantum system. This is one point where $\su2$ and $\uone$ differ: crucially we do \emph{not} have this bipartite structure in the $\uone$ case because all of its irreps are one dimensional. In the $\su2$ case we visualise the bipartite-like structure graphically by adding virtual vertices to the ends of a link, intended to represent these two different subsystems: we now picture an arbitrary state $|\psi\rangle$ of a single link as  
\begin{center}
