...
The quantum degree of freedom we associate with each edge is then the hilbert space $\mathcal{h}\cong L^2(G)$. Thus the total hilbert space for our system is
\begin{equation}
	\mathcal{H} = \bigotimes_{e\in E} \mathcal{h}.
\end{equation}
...
This prescription generalises in a natural way to arbitrary graphs and even allows us to carry out interpolations that would otherwise require the solution of a nonlinear equation. 
...
\begin{equation}
	\tau_{jk}^\ell(U) \equiv \langle W_j| U\otimes \cdots \otimes U |W_k\rangle.
\end{equation}
...
\end{equation}
From this expression we immediately dedude that for the RHS to be nonzero we need $\ell' = \ell$ (just use the left invariance of the haar measure). To deduce the rest of the result we simply exploit the left invariance of the haar measure to change variables to $V = \Phi U$, where $\Phi = e^{i\phi \sigma^z}$:
\begin{multline}
		({\tau}^{\ell'}_{j'k'}, {\tau}^{\ell}_{jk}) = \int dV\, \langle W_{k'}| \underbrace{V^\dag \otimes \cdots \otimes V^\dag}_{\text{$2\ell'$ times}}  (\Phi^\dag\otimes \cdots \otimes \Phi^\dag)|W_{j'}\rangle \langle W_j|(\Phi\otimes \cdots \otimes \Phi) \underbrace{V\otimes \cdots \otimes V}_{\text{$2\ell$ times}} |W_k\rangle \\ = e^{-i\phi(\ell - j' - (\ell + j')}e^{i\phi(\ell - j - (\ell + j)} ({\tau}^{\ell'}_{j'k'}, {\tau}^{\ell}_{jk}) = e^{2i\phi (j'-j)} ({\tau}^{\ell'}_{j'k'}, {\tau}^{\ell}_{jk}).
\end{multline}
Since this is true for all $\phi \in [0, 2\pi)$ we find that $j=j'$ in order for the inner product to be nonzero.
...
\end{equation}
Our task is to work out the unitary operation realising this decomposition.

Here we want to decomp
